...
| 字段 | 必填 | 可选值 / 写法 | 显示在哪里 |
|---|---|---|---|
| `description` | 建议 | 一句话 | 笔记列表、首页"最近更新"、搜索结果 |
| `type` | 建议 | 任意文字，如 `思考` / `论文阅读` / `有的没的`；写一个新的就多一个筛选按钮 | 信息栏、笔记列表、筛选按钮 |
| `tags` | 建议 | 任意标签，2–4 个为宜（见[标签](#标签)） | 笔记列表、页面底部、标签页 |
...
类型不需要登记，下面是现在用到的几种；想加新类型直接写就行，按钮按篇数从多到少排列。
...
标签自由填写，写什么就出现什么：[标签](../tags.md)页自动列出所有用到的标签及篇数，按篇数从多到少排列，没人用的自动消失。
尽量用标签表示**主题或方法**，复用已有的标签、不造近义词，一篇 2–4 个为宜。

`zensical.toml` 里的 `[project.extra.tag_groups]` 是可选的，只决定标签页上的分组（主题 / 方法 / 兴趣），不在分组里的标签归到"其他"。
运行 `python scripts/site_data.py` 可以检查所有文章的 front matter 有没有写坏。
