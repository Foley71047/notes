---
description: 本站的写作模板：笔记与定理词条的元数据、要点框、悬停预览、数学公式、提示框、PDF 嵌入、代码块、脚注、选项卡与 Mermaid 图表的写法，复制即可用。
---
...
| `docs/theorems/` | 定理词条：一个具体的定理、不等式或结论 | [定理](../theorems/index.md)页自动列出，按拼音排序 |
| `docs/about/` | 关于、写作模板 | — |

新建文件后**不需要改导航或任何列表**：笔记列表、定理速查表、[标签](../tags.md)页、首页"最近更新"和"引用本定理的笔记"都在构建时根据元数据自动生成。页面上的信息栏（类型、更新日期、阅读时长……）也全部来自元数据，正文里不用写。
...
tags:
  ...
  - 纠缠
prerequisites:                        # 可选
  - di-primer.md                      # 站内页面：写相对路径，显示为该页标题的链接
  - ../theorems/chsh-inequality.md
  - 线性代数                           # 其他：原样显示
---
...
| `type` | 是 | `学习` / `研究` / `论文阅读` / `随想` | 信息栏、笔记列表、筛选按钮 |
| `tags` | 是 | 2–4 个，只能用[标签清单](#标签)里的 | 笔记列表、页面底部、标签页 |
| `prerequisites` | 否 | 列表；站内页面写相对 `.md` 路径，其他写文字 | 信息栏"前置知识" |
| `reading_time` | 否 | 整数（分钟），覆盖自动估算，比如 PDF 教材 | 信息栏、笔记列表 |

更新日期取自 git 最后一次提交，阅读时长按字数和公式数自动估算（中文每分钟 400 字、英文每分钟 200 词、独立公式每个 15 秒），都不用手写。

### 类型怎么选

| 类型 | 写什么 | 例子 |
|---|---|---|
| **学习** | 教科书式的已知知识，系统整理，可以独立复习。尽量按"直观图像 → 严格定义 → 带数字的例题 → 一句话总结"组织 | 课程笔记、教材读书笔记 |
| **研究** | 和正在进行的研究直接相关：研究推导（定理的完整证明、计算细节）、研究总结（阶段性梳理、与导师讨论后的整理） | 项目推导、组会总结 |
| **论文阅读** | 一篇论文的问题、方法、结论与疑问 | 某篇 PRL 的阅读笔记 |
| **随想** | 零散的想法、学业以外的探索 | 一个没想清楚的问题 |
...
每个词条是**一个具体的**定理、不等式或结论（比如"CHSH 不等式""Tsirelson 界"），不收录"纠缠""Bell 非局域性"这样的领域性概念——那些是标签。

在 `docs/theorems/` 下新建文件（如 `docs/theorems/tsirelson-bound.md`），按下面的结构写。标题下的英文名和标签、文末的"引用本定理的笔记"都会自动生成，不要自己写。

````markdown title="定理词条模板"
---
en: Tsirelson's bound                              # 英文名
statement: '量子力学中 CHSH 值满足 $\lvert S\rvert\le 2\sqrt2$'   # 一行核心陈述，显示在速查表里
...
在任何页面里链接到定理词条，鼠标悬停在链接上就会弹出预览卡片，显示词条的英文名、标签和"陈述"框（公式也会排版）：

```markdown title="链接到定理"
量子力学允许的最大值由 [Tsirelson 界](../theorems/tsirelson-bound.md) 给出。
```

量子力学允许的最大值由 [Tsirelson 界](../theorems/tsirelson-bound.md) 给出；局域模型的上界见 [CHSH 不等式](../theorems/chsh-inequality.md)。（把鼠标放到这两个链接上试试。）

这是 Zensical 的"即时预览"功能（`zensical.toml` 里的 `zensical.extensions.preview`），对所有指向 `docs/theorems/` 的站内链接自动生效。只有**写成链接**的定理名才有预览；手机上长按链接也能看到。同时，笔记里链接过的定理，会自动出现在该词条末尾的"引用本定理的笔记"里。
